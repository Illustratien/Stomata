...
Ntu_dlist <- map(ntu_file,~{
  data.table::fread(file.path("result/Ntu/",.x)) %>% 
    ...
    mutate(
      pic_name=as.character(pic_name),
    ...
    rename(stomata.cx=boundingbox_x,stomata.cy=boundingbox_y,
           boundingbox_length=boundingbox_width,
...
folder_vec <-map_chr(Ntu_dlist,~{
  .x$pic_name %>% 
    strsplit("_") %>% map_depth(.,1,~{.x[1]}) %>%
    unlist() %>% unique() %>% paste(.,collapse="_")
})
...
walk(1:length(folder_vec),function(foldid){
  ...
  if(!identical(in_pic, character(0))){
